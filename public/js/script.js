...
let leftEndBack = center - 3 >= 0? center - 3 : undefined;
let leftEnd = center - 2 >= 0? center - 2 : undefined;
let leftMid = center - 1 >= 0? center - 1 : undefined;
let rightMid = center + 1 < sliders.length? center + 1 : undefined;
let rightEnd = center + 2 < sliders.length? center + 2 : undefined;
let rightEndBack = center + 3 < sliders.length? center + 3 : undefined;
let touchstartX = 0
let touchendX = 0
...
document.addEventListener('DOMContentLoaded', () => {
    const slides = document.querySelectorAll('.lower-section');
    const colors = ['#dc3545', '#e83e8c', '#6f42c1', '#17a2b8', '#fd7e14', '#ffc107', '#28a745'];

    slides.forEach(slide => {
        slide.style.backgroundColor = getRandomColorFromList(colors);
    });
});
